...
Dims SPPF::init(Dims _stInPut)
{
	Dims dim_cv1 = cv1->init(_stInPut);

	dim_cv1.d[1] *= 4;

	dim_cv1 = cv2->init(dim_cv1);

	return dim_cv1;

}
int SPPF::forwardEx(void* _pInData, Dims _stInPut, void* _pOutData, Dims &_stOutPut, void *_pBuffer)
{
	Dims stDimCv1, stDimCv2, stDimCv3, stDimCv4;
	void *buffer1 = _pBuffer;
	void *tmp_buf = (char*)_pBuffer + get_bolck_size(_stInPut) * sizeof(float);
	...
	cv2->forwardEx(buffer7, stDimCat3, _pOutData, _stOutPut, tmp_buf);
#endif
	return 0;
}